    ...
    char **argv)		/* Argument strings. */
{
	...
	if (CkTextDLineInfo(textPtr, &index1, &x, &y, &width, &height, &base)
	    ...
	    sprintf(buffer, "%d %d %d %d %d", x, y, width,
		    height, base);
	    Tcl_SetResult(interp, buffer, TCL_VOLATILE);
	}
	...
	CkTextPrintIndex(&index1, buffer);
	Tcl_SetResult(interp, buffer, TCL_VOLATILE);
    ...
    CkEvent *eventPtr)		/* Information about event. */
{
	...
	Tcl_EventuallyFree((ClientData) textPtr, (Ck_FreeProc *) DestroyText);
