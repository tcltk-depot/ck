 ...
 *	In this case, an error message is left in interp's result.
 *
 * Side effects:
 *	None.
 *
 ...
 *	error message will be left in interp's result.  The memory at
				 ...
				 * 0 and TK_MAX_PRIO. */
{
	...
	while (*src != ':') {
	    if ((*src == '\0') || (*src == '\n')) {
		sprintf(buffer, "missing colon on line %d", lineNum);
	...
	*dst = '\0';
	...
	while (*src != '\n') {
	    if (*src == '\0') {
		sprintf(buffer, "missing newline on line %d", lineNum);
		Tcl_SetResult(interp, buffer, TCL_VOLATILE);
		return TCL_ERROR;
	    }
	    if ((src[0] == '\\') && (src[1] == '\n')) {
 ...
 *	The return value is a standard Tcl return code.  In the case of
 *	an error in parsing string, TCL_ERROR will be returned and an
 *	error message will be left in interp's result.
 *
 * Side effects:
 *	None.
 *
