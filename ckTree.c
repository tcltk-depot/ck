    ...
    char **argv)		/* Argument strings. */
{
    ...
    Tcl_Preserve((ClientData) treePtr);
    c = argv[1][0];
	...
	if ((c == 'c') && (argv[2] != NULL) &&
	    ...
	    Tcl_SetResult(interp, (nodePtr->flags & SELECTED) ?
			  "1" : "0", TCL_STATIC);
    ...
    Tcl_Release((ClientData) treePtr);
    return result;
 ...
 *	to interp's result;  otherwise tag is added to nodePtr's
 *	list of tags.
 *
 *--------------------------------------------------------------
 */
 ...
 *	returned in interp's result.  If newTag is NULL, then
 *	the normal interp's result is an empty string.  If an error
 *	occurs, then interp's result will hold an error message.
 *
 * Side effects:
