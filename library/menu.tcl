...
proc ckMbPost {w {x {}} {y {}}} {
    global ckPriv
    if {([$w cget -state] eq "disabled") || ($w eq $ckPriv(postedMb))} {
	return 0
    }
    ...
    if {([$w cget -indicatoron] == 1) && ([$w cget -textvariable] ne "")} {
	if {$y eq ""} {
...
proc ckMenuEscape menu {
    if {[winfo class [winfo parent $menu]] ne "Menu"} {
	ckMenuUnpost $menu
    } else {
...
proc ck_popup {menu x y {entry {}}} {
    global ckPriv
    if {($ckPriv(popup) ne "") || ($ckPriv(postedMb) ne "")} {
	ckMenuUnpost {}
    }
