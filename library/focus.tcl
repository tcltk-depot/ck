...
proc ck_focusNext w {
	...
	while {1} {
	    ...
	    set cur $parent
            if {$parent eq $ckPriv(restrictWindow)} break
	    if {[winfo toplevel $cur] eq $cur} {
		break
	    }
...
proc ckFocusOK w {
    ...
    if {$ckPriv(restrictWindow) ne ""} {
        if {[winfo toplevel $w] eq [winfo toplevel $ckPriv(restrictWindow)]} {
	...
	if {$value == 0} {
	    ...
	    if {[winfo class $w] eq "Listbox" && [$w size] == 0} {
		return 0
	    }
	    return 1
	} else {
