    ...
    char **argv;		/* Argument strings. */
{
	...
	sprintf(buffer, "%d %d %d %d", prevX, prevY, x - prevX, y - prevY);
	...
	masterPtr = GetGridBag(master);
	if (argc == 3) {
	    Tcl_SetResult(interp, (masterPtr->flags & DONT_PROPAGATE) ?
			  "0" : "1", TCL_STATIC);
	    return TCL_OK;
	}
	...
	sprintf(buffer, "%d %d", masterPtr->layoutCache->width,
 ...
 *	returned and interp's result is set to contain an error message.
 *
 * Side effects:
	...
	for (i = numWindows; i < argc; i+=2) {
		...
		if (j == 0) {
			...
			Tcl_SetResult(interp,
			    "Window can't be managed in itself", TCL_STATIC);
			return TCL_ERROR;
		    }
