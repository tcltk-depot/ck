    ...
    char **argv)		/* Argument strings. */
{
	...
	if (argc == 2) {
	    Tcl_SetResult(interp, (mainPtr->flags & CK_REVERSE_KLUDGE) ?
			  "1" : "0", TCL_STATIC);
	} else if (argc == 3) {
	...
	Tcl_DStringFree(&buffer);
	if (ret != OK) {
	    Tcl_SetResult(interp, "screen dump failed", TCL_STATIC);
	    return TCL_ERROR;
	}
	return TCL_OK;
#else
	Tcl_SetResult(interp, "screen dump not supported by this curses",
		      TCL_STATIC);
	return TCL_ERROR;
#endif
...
#define SETUP(name) \
	...
	if (winPtr->parentPtr != NULL)
	    ...
	    && (length >= 4)) {
	SETUP("reqheight");
	sprintf(buffer, "%d", winPtr->reqHeight);
	Tcl_SetResult(interp, buffer, TCL_VOLATILE);
    } else if ((c == 'r') && (strncmp(argv[1], "reqwidth", length) == 0)
	    && (length >= 4)) {
	SETUP("reqwidth");
	sprintf(buffer, "%d", winPtr->reqWidth);
	Tcl_SetResult(interp, buffer, TCL_VOLATILE);
	...
	Tcl_SetResult(interp, buffer, TCL_VOLATILE);
	    ...
	    && (length >= 7)) {
	SETUP("screenheight");
	sprintf(buffer, "%d", winPtr->mainPtr->winPtr->height);
	Tcl_SetResult(interp, buffer, TCL_VOLATILE);
    } else if ((c == 's') && (strncmp(argv[1], "screenwidth", length) == 0)
	    && (length >= 7)) {
	SETUP("screenwidth");
	sprintf(buffer, "%d", winPtr->mainPtr->winPtr->width);
	Tcl_SetResult(interp, buffer, TCL_VOLATILE);
	...
	SETUP("width");
	sprintf(buffer, "%d", winPtr->width);
	Tcl_SetResult(interp, buffer, TCL_VOLATILE);
    } else if ((c == 'x') && (argv[1][1] == '\0')) {
	SETUP("x");
	sprintf(buffer, "%d", winPtr->x);
	Tcl_SetResult(interp, buffer, TCL_VOLATILE);
    } else if ((c == 'y') && (argv[1][1] == '\0')) {
	SETUP("y");
	sprintf(buffer, "%d", winPtr->y);
	Tcl_SetResult(interp, buffer, TCL_VOLATILE);
    } else {
	Tcl_AppendResult(interp, "bad option \"", argv[1],
